...
**Measured visibility has one rule worth knowing**: an unfilled zone occupies no space, so what the detector measures is an artifact of there being nothing to see, not a statement about the slot. `_onVisibilityInfo` ignores any measurement whose `size.isEmpty`, and the last real measurement stands. Removing that guard freezes the countdown of every zone the moment it goes unfilled, and it never asks for another ad.
